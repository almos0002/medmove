...
import { z } from 'zod'
import { listIncomingTransferRequests } from '@/server/functions/transfers'
...
export const Route = createFileRoute('/org/requests/incoming')({
  ...
  beforeLoad: async ({ context }) => {
    const session = (
      context as {
        session?: {
          primaryOrg?: { id: string; canListMedicine: boolean } | null
        }
      }
    ).session
    ...
    if (!session.primaryOrg.canListMedicine) {
      throw redirect({ to: '/org' })
    }
  ...
  loader: ({ context, deps }) => {
    ...
    return listIncomingTransferRequests({
      data: {
  ...
  head: pageHead({ title: "Incoming requests", noindex: true }),
  component: OrgIncomingRequestsPage,
})
...
function OrgIncomingRequestsPage() {
  ...
  const pendingCount = items.filter(
    (r) => r.request.status === 'pending_seller',
  ).length
  ...
  return (
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <EmptyState
          icon={Inbox}
          title={hasFilters ? 'No matches' : 'No incoming requests'}
          description={
            hasFilters
              ? 'Try widening your filters above to see more requests.'
              : 'Once a buyer requests one of your listings (and an admin approves it), it will appear here for you to accept or decline.'
          }
          action={
            hasFilters ? (
              <Button
